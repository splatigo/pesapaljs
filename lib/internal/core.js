...
var EWALLET = new models.PaymentMethod("PESAPAL", "PesaPal", "PesaPal Account", null, null),
    ...
    Airtel_UG = new models.PaymentMethod("ZAP_UG", "Airtel_UG", "Airtel mobile payment", "111222", null),
    AIRTEL = new models.PaymentMethod("ZAP", "Airtel Money", "Airtel mobile payment", "PESAPAL", null),
    VISA = new models.PaymentMethod("VISA", "Visa", "Visa Credit / Debit Card", null, "CREDITCARD"), // PesaPal uses CREDITCARD
    ...
    MTN_UG =new models.PaymentMethod("MTNMONEY_UG", "MTN Money_UG", "MTN mobile payment", "PESAPAL", null);
                                      
exports.SUPPORTED_PAYMENT_METHODS = [EWALLET, MPESA, Airtel_UG,AIRTEL, VISA, MASTERCARD,MTN_UG];
...
exports.setup = function (key, secret, debug) {
    ...
    CUSTOM_UI_PAYMENT_METHODS = [MPESA.tag,Airtel_UG.tag, AIRTEL.tag, VISA.tag, MASTERCARD.tag,MTN_UG.tag];
...
/*
    var url = new URI(PAYMENT_STATUS_URL);
    if (statusOnly == false) {
        url = new URI(PAYMENT_DETAILS_URL);
    }
    if (type) {
        url.addQuery(DATA_KEYS.type, type);
    }
    if (reference) {
        url.addQuery(DATA_KEYS.reference, reference);
    }
    if (transaction) {
        url.addQuery(DATA_KEYS.transaction, transaction);
    }*/
    ...
    var req = signer.sign({
        action: "GET",
        path: PAYMENT_DETAILS_URL,
        parameters: {
            pesapal_transaction_tracking_id: transaction,
   ...
   /* var oauth = {
    ...
    // request.get({url: req.signed_url, jar: cookieJar, headers: requestHeaders}
...
exports.prepareOrder = function (order, paymentMethod, callback) {
    ...
    if(CUSTOM_UI_PAYMENT_METHODS.indexOf(paymentMethod.tag) == -1) {
        throw new Error("Payment method not supported. Only MPesa, Airtel_UG, Airtel Money or Bank cards(Visa & MasterCard) are supported");
    }
